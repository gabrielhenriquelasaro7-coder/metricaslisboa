...
import { Search, Loader2, ImageOff, Play, ChevronLeft, ChevronRight, ShoppingCart, TrendingUp, DollarSign, Target, Calendar, RefreshCw } from 'lucide-react';
...
export default function Creatives() {
  ...
  const filteredCreatives = useMemo(() => {
    return ads.filter(ad => {
      if (search) {
        ...
        return ad.name.toLowerCase().includes(searchLower) || ad.headline?.toLowerCase().includes(searchLower) || false || ad.primary_text?.toLowerCase().includes(searchLower) || false || getCampaignName(ad.campaign_id).toLowerCase().includes(searchLower) || getAdSetName(ad.ad_set_id).toLowerCase().includes(searchLower);
      }
      return true;
    }).filter(ad => {
  ...
  const getStatusBadge = useCallback((status: string) => {
    const statusMap: Record<string, {
      ...
      ACTIVE: {
        label: 'Ativo',
        className: 'bg-metric-positive/20 text-metric-positive'
      },
      PAUSED: {
        label: 'Pausado',
        className: 'bg-metric-warning/20 text-metric-warning'
      },
      DELETED: {
        label: 'Deletado',
      ...
      <div className="p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8 animate-fade-in overflow-x-hidden">
        ...
        <div className="flex flex-col gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-xs sm:text-sm flex items-center flex-wrap gap-2">
              ...
              {selectedProject && <Badge variant="outline" className="text-xs border-primary/30 text-primary">
                  {isEcommerce ? 'E-commerce' : isInsideSales ? 'Inside Sales' : 'PDV'}
                </Badge>}
              {dateRange?.from && dateRange?.to && <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded border border-primary/20">
                  {dateRange.from.toLocaleDateString('pt-BR')} - {dateRange.to.toLocaleDateString('pt-BR')}
                </span>}
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={() => syncCreativesHD()} disabled={syncing} variant="outline" className="gap-2">
              {syncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4 border-2" />}
              <span className="hidden sm:inline">Sincronizar Criativos HD</span>
              <span className="sm:hidden text-xs">Sync HD</span>
            </Button>
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-3 sm:gap-4">
          <div className="relative flex-1 min-w-0 max-w-full sm:max-w-md">
            ...
            <Input placeholder="Buscar criativos..." value={search} onChange={e => {
        ...
        <div className={cn("grid gap-2 sm:gap-4", isEcommerce ? "grid-cols-2 sm:grid-cols-3 lg:grid-cols-6" : "grid-cols-2 sm:grid-cols-2 lg:grid-cols-4")}>
          ...
          {isInsideSales && <div className="glass-card p-2.5 sm:p-4">
              <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                <DollarSign className="w-3 h-3 sm:w-4 sm:h-4 text-primary" />
          ...
          {isPdv && <div className="glass-card p-2.5 sm:p-4">
              <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                <DollarSign className="w-3 h-3 sm:w-4 sm:h-4 text-primary" />
        ...
        {usingFallbackData && filteredCreatives.length === 0 && <div className="glass-card p-12 text-center">
            ...
            {dataDateRange && <p className="text-sm text-muted-foreground">
        ...
        {!usingFallbackData && filteredCreatives.length === 0 && <div className="glass-card p-12 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              {campaignFilter !== 'all' || adSetFilter !== 'all' ? 'Tente ajustar os filtros para ver mais criativos.' : 'Sincronize seus dados para ver os criativos das suas campanhas.'}
            </p>
            {campaignFilter === 'all' && adSetFilter === 'all' && <Button onClick={() => syncCreativesHD()} disabled={syncing} className="bg-primary hover:bg-primary/90">
        ...
        {paginatedCreatives.length > 0 && <div className="glass-card overflow-hidden">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border bg-secondary/30">
                    ...
                    {isEcommerce && <th className="text-right py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">ROAS</th>}
                    ...
                    {isEcommerce && <th className="text-right py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wider hidden xl:table-cell">Ticket</th>}
                ...
                return <tr key={ad.id} className="border-b border-border/50 hover:bg-secondary/20 cursor-pointer transition-colors" onClick={() => navigate(`/ad/${ad.id}`)}>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3 min-w-0">
                            {/* Creative Thumbnail */}
                            <div className="w-12 h-12 rounded-lg bg-secondary/50 flex-shrink-0 overflow-hidden border border-border/50">
                              <CreativeImage projectId={selectedProject?.id} adId={ad.id} cachedImageUrl={ad.cached_image_url} creativeImageUrl={ad.creative_image_url} creativeThumbnail={ad.creative_thumbnail} alt={ad.name} className="w-full h-full object-contain" fallbackClassName="bg-secondary/50" />
                            </div>
                            <div className="min-w-0">
                              <p className="font-medium truncate max-w-[200px]">{ad.name}</p>
                              {ad.headline && <p className="text-xs text-muted-foreground truncate max-w-[200px]">{ad.headline}</p>}
                            </div>
                          </div>
                        </td>
                        ...
                        {isEcommerce && <td className="py-3 px-4 text-right">
                            <Badge variant="outline" className={cn("font-semibold", ad.roas >= 5 && "bg-metric-positive/20 text-metric-positive border-metric-positive/30", ad.roas >= 3 && ad.roas < 5 && "bg-metric-warning/20 text-metric-warning border-metric-warning/30", ad.roas < 3 && ad.roas > 0 && "bg-metric-negative/20 text-metric-negative border-metric-negative/30")}>
                              {ad.roas.toFixed(2)}x
                            </Badge>
                          </td>}
                        ...
                        {isEcommerce && <td className="py-3 px-4 text-right hidden xl:table-cell">
            ...
            {/* Pagination */}
            {totalPages > 1 && <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-secondary/10">
