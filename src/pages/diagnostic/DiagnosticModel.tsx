...
const TRAVAS = [
    ...
    { id: '00', icon: <SearchX className="w-6 h-6" />, title: 'Cegueira', desc: 'Não há dados confiáveis — cegueira operacional', color: 'border-border bg-muted/30', badge: 'text-muted-foreground bg-muted' },
];
...
const TOC_STEPS = [
    ...
    { num: '5', label: 'REPETIR', desc: 'Quando a restrição muda, voltar ao passo 1', color: 'bg-muted-foreground' },
];
        ...
        <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
                    ...
                    {TRAVAS.map((trava) => (
                        <div key={trava.id} className={cn("p-5 rounded-xl border flex flex-col gap-3 group transition-all hover:scale-[1.02] duration-300", trava.color)}>
                            <div className="flex items-center justify-between">
                                <div className={cn("p-2 rounded-xl bg-card border border-border", trava.badge)}>
                                    {trava.icon}
                                </div>
            ...
            <section className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    {TOC_STEPS.map((step) => (
                        <div key={step.num} className="relative p-5 bg-card border border-border rounded-xl space-y-3 group hover:border-foreground/10 transition-all">
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            ...
                            <div key={m.abbr} className="p-6 bg-card border border-border rounded-2xl flex flex-col items-center justify-center text-center space-y-4 hover:border-amber-500/20 transition-all shadow-md">
                                <div>
                    ...
                    <div className="p-6 md:p-8 bg-card border border-border rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-md">
                        ...
                        <div className="flex-1 p-5 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-start gap-3 w-full">
                            ...
                            <p className="text-xs text-amber-600 leading-relaxed font-bold">
                                Se share necessário &gt; 20-30% → <span className="text-amber-600 font-black">sinal vermelho</span>. O mercado pode ser a própria restrição.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section className="space-y-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                    {ROTINA_STEPS.map((step) => (
                        <div key={step.num} className="p-5 bg-card border border-border rounded-xl space-y-3 hover:border-foreground/10 transition-all group">
