...
export default function Dashboard() {
  ...
  } = useHiddenMetrics("dashboard", businessModel as any);

  const { config: metricConfig, loading: metricConfigLoading } = useProjectMetricConfig(
    isCustom ? selectedProject?.id : null,
  );
  ...
  // Preparar dados para o novo FunnelChart
  const funnelData = useMemo(() => {
    return [
      { name: "impressions", value: metrics.totalImpressions },
      { name: "clicks", value: metrics.totalClicks },
      { name: "conversions", value: metrics.totalConversions },
      { name: "sales", value: metrics.totalSalesConversions },
      { name: "leads", value: metrics.totalLeadsConversions },
    ].filter((d) => d.value > 0);
  }, [metrics]);
      ...
      <div className="relative min-h-screen overflow-x-hidden w-full max-w-full">
        ...
        <div className="relative z-10 p-3 sm:p-6 lg:p-8 space-y-3 sm:space-y-6 lg:space-y-8 animate-fade-in overflow-x-hidden w-full">
          <div className="flex flex-col gap-3 sm:gap-4">
            <div>
              <h1 className="text-lg sm:text-2xl lg:text-3xl font-bold font-space-grotesk">{t("dashboard.title")}</h1>
                ...
                {hasSelectedProject && selectedProject && (
                  <div data-tour="pdf-export">
                    <PDFBuilderDialog
                      ...
                      currency={selectedProject.currency || "BRL"}
                      currentPeriod={
                        getDateRangeFromPreset(selectedPreset, projectTimezone) || {
          ...
          {activeProjects.length === 0 && !loading ? (
            ...
            <>
              {hasSelectedProject && (
                <AccountBalanceCard projectId={selectedProject?.id || null} currency={selectedProject?.currency} />
              )}
              ...
              <div className="flex items-center justify-end gap-2 flex-wrap">
                ...
                <Label
                  ...
                  className="text-[11px] sm:text-sm text-muted-foreground cursor-pointer"
                >
              ...
              {showComparison && hasSelectedProject && (
                <PeriodComparison
                  metrics={[]} /* PeriodComparison atualizado nao usa mais currentMetrics e previousMetrics diretamente da forma antiga, ou precisa ser revertido se o PeriodComparison nao foi atualizado */
                  /* Assumindo que você manteve o PeriodComparison.tsx que te enviei anteriormente, ele usa 'metrics' array. */
                  /* Se nao, reverta para usar o antigo. Vou assumir o antigo aqui pois você só pediu para corrigir Funnel e HeatMap */
                  {...{
                    currentMetrics: metrics,
                    previousMetrics,
                    businessModel: businessModel || null,
                    currentPeriodLabel: t("comparison.current"),
                    previousPeriodLabel: t("dashboard.previous"),
                    currency: selectedProject?.currency || "BRL",
                    hiddenMetrics,
                  }}
                />
              ...
              {/* Ajuste importante: Se o PeriodComparison novo foi aplicado, use este bloco comentado abaixo. 
               Se o antigo (Dashboard.tsx original) ainda espera props antigas, o código acima tenta manter compatibilidade. 
               Mas como você pediu para corrigir o erro do Funnel e HeatMap, vou focar neles.
            */}

              <div data-tour="metrics">
                <div className="flex items-center gap-2 mb-2 sm:mb-4">
                  <div className="w-1 h-4 sm:h-6 bg-gradient-to-b from-primary to-primary/50 rounded-full" />
                  <h2 className="text-sm sm:text-lg font-semibold text-foreground font-space-grotesk">
                ...
                <div className="metric-grid-mobile">
                  ...
                  {!isMetricHidden("impressions") && (
                    <SparklineCard
                      title={t("metrics.impressions")}
                      value={formatNumberCompact(metrics.totalImpressions)}
                  ...
                  {!isMetricHidden("clicks") && (
                    <SparklineCard
                      ...
                      value={formatNumberCompact(metrics.totalClicks)}
              ...
              {hasSelectedProject &&
                profileVisitsData.hasProfileVisitCampaigns &&
                !isMetricHidden("profile_visits") && (
                  <div>
                    <div className="flex items-center gap-2 mb-2 sm:mb-4">
                      <div className="w-1 h-4 sm:h-6 bg-gradient-to-b from-pink-500 to-pink-500/50 rounded-full" />
                      <h2 className="text-sm sm:text-lg font-semibold text-foreground flex items-center gap-1 sm:gap-2 flex-wrap font-space-grotesk">
              ...
              {hasSelectedProject && (
                <div>
                  <div className="flex items-center gap-2 mb-2 sm:mb-4">
                    ...
                    <h2 className="text-sm sm:text-lg font-semibold text-foreground flex items-center flex-wrap gap-1 font-space-grotesk">
                  ...
                  {isEcommerce && (
                    <div className="metric-grid-results">
                      ...
                      {!isMetricHidden("conversions") && !isMetricHidden("purchases") && (
                        <SparklineCard
                          title={t("metrics.purchases")}
                          value={formatNumber(metrics.totalSalesConversions || metrics.totalConversions)}
                          change={changes?.conversions}
                          icon={ShoppingCart}
                          sparklineData={
                            sparklineData.purchases.length > 0 ? sparklineData.purchases : sparklineData.conversions
                          }
                          tooltip={t("metrics.purchasesTooltip")}
                        />
                      )}
                      {!isMetricHidden("conversion_value") && (
                        <SparklineCard
                  ...
                  {isInsideSales &&
                    (() => {
                      ...
                      return (
                        <div className="metric-grid-results">
                          {!isMetricHidden("conversions") && !isMetricHidden("leads") && (
                            <SparklineCard
                              title={t("metrics.leads")}
                              value={formatNumber(totalLeads)}
                              change={changes?.conversions}
                              icon={Users}
                              sparklineData={
                                sparklineData.leads.length > 0 ? sparklineData.leads : sparklineData.conversions
                              }
                              className="border-l-4 border-l-chart-1"
                              tooltip={t("metrics.totalResults")}
                            />
                          )}
                          {!isMetricHidden("cpa") && (
                            <SparklineCard
                  ...
                  {isPdv && (
                    <div className="metric-grid-results">
                      {!isMetricHidden("conversions") && (
                        <SparklineCard
                          title={t("metrics.visits")}
              ...
              {/* Funnel Chart CORRIGIDO */}
              {hasSelectedProject && <FunnelChart data={funnelData} />}

              {/* Geographic Heat Map CORRIGIDO */}
              <GeographicHeatMap data={demographicData?.region || []} />
