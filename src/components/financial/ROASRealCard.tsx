    ...
    <Card className={cn('relative overflow-hidden', config.borderColor)}>
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div>
            <CardDescription className="text-xs mb-1">{periodLabel}</CardDescription>
            <CardTitle className="text-lg flex items-center gap-2">
              ROAS Real
              <Badge className={cn('font-medium', config.color, config.bgColor)}>
                {config.label}
              </Badge>
            </CardTitle>
          </div>
          <div className={cn('p-3 rounded-xl', config.bgColor)}>
            <TrendingUp className={cn('w-6 h-6', config.color)} />
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center py-4">
          <div className={cn('text-5xl font-bold tracking-tight', config.color)}>
            {roas.toFixed(2)}x
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Receita do CRM</p>
                <p className="font-semibold">{formatCurrency(crmRevenue)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-semibold">{formatCurrency(adSpend)}</p>
              </div>
            </div>
          </div>
        </div>
