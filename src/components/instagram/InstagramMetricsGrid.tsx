...
import { Eye, Heart, MessageCircle, Share2, Bookmark, Users, UserPlus, TrendingUp, MousePointer, Calendar } from 'lucide-react';
...
export default function InstagramMetricsGrid({ metrics, followersCount }: Props) {
  const items = [
    ...
    { label: 'Comentários', sublabel: 'No período', value: fmt(metrics.totalComments), icon: MessageCircle, color: 'text-orange-500', bg: 'bg-orange-500/10' },
    { label: 'Salvos', sublabel: 'No período', value: fmt(metrics.totalSaves), icon: Bookmark, color: 'text-yellow-500', bg: 'bg-yellow-500/10' },
    { label: 'Compartilhamentos', sublabel: 'No período', value: fmt(metrics.totalShares), icon: Share2, color: 'text-pink-500', bg: 'bg-pink-500/10' },
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-xs text-muted-foreground px-1">
        ...
        <span>Período analisado: <strong className="text-foreground">Últimos 30 dias</strong></span>
      </div>
