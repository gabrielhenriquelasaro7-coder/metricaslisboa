...
import { FileText, Download, Loader2, Calendar, Upload, X, LayoutTemplate, PieChart, Users, Smartphone, Globe, TrendingUp, Trophy, Settings, Eye } from 'lucide-react';
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden max-w-xl mx-auto">
      ...
      <div className="px-4 py-3 flex items-center justify-between" style={{ backgroundColor: primaryColor }}>
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[10px] md:text-xs text-white/80">{fmtDateRange(activePeriod.since, activePeriod.until)}</p>
        </div>
        {logoUrl && <img src={logoUrl} alt="Logo" className="h-6 md:h-8 w-auto max-w-12 md:max-w-16 object-contain ml-2 shrink-0" />}
      </div>
      
      <div className="p-3 md:p-4 space-y-3 md:space-y-4">
        <p className="text-[9px] md:text-[10px] text-gray-400">Gerado em {format(new Date(), "dd/MM/yyyy 'às' HH:mm")}</p>
        
        {/* Summary */}
        <div>
          <h2 className="text-[10px] md:text-xs font-semibold text-gray-900 mb-2">Resumo Executivo</h2>
          <div className="rounded border p-2 text-[10px] md:text-xs" style={{ borderColor: primaryColor, backgroundColor: `${primaryColor}10` }}>
        ...
        {selGeneral.length > 0 && totals && (
          <div>
            <h2 className="text-[10px] md:text-xs font-semibold text-gray-900 mb-2">Métricas Gerais</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-1 md:gap-2">
              {selGeneral.map(key => {
                const def = GENERAL_METRICS.find(m => m.key === key);
                if (!def) return null;
                const value = (totals as unknown as Record<string, number>)[key];
                return (
                  <div key={key} className="bg-gray-50 rounded p-1.5 md:p-2 border border-gray-100">
                    <p className="text-[9px] md:text-[10px] text-gray-500">{def.label}</p>
                    <p className="text-xs md:text-sm font-bold text-gray-900">{fmtValue(value, def.type, currency)}</p>
                  </div>
                );
              })}
            </div>
        ...
        {/* Result Metrics */}
        {selResult.length > 0 && totals && (
          <div>
            <h2 className="text-[10px] md:text-xs font-semibold text-gray-900 mb-2">Métricas de Resultado</h2>
            <div className="grid grid-cols-2 gap-1 md:gap-2">
              {selResult.map(key => {
                const def = resultDefs.find(m => m.key === key);
                if (!def) return null;
                const value = (totals as unknown as Record<string, number>)[key];
                return (
                  <div key={key} className="rounded p-1.5 md:p-2 border-2" style={{ borderColor: primaryColor, backgroundColor: `${primaryColor}10` }}>
                    <p className="text-[9px] md:text-[10px]" style={{ color: primaryColor }}>{def.label}</p>
                    <p className="text-sm md:text-lg font-bold" style={{ color: primaryColor }}>{fmtValue(value, def.type, currency)}</p>
                  </div>
                );
              })}
            </div>
        ...
        {includeChart && chartData.length > 0 && (
          <div>
            <h2 className="text-[10px] md:text-xs font-semibold text-gray-900 mb-2">
              {primaryMetricDef?.label || 'Métrica'} {showSecondaryMetric ? `vs ${secondaryMetricDef?.label || ''}` : ''}
            </h2>
            <div id="pdf-chart-preview" className="bg-white rounded border p-2" style={{ height: isMobile ? 140 : 180 }}>
              <ResponsiveContainer width="100%" height="100%">
                {chartType === 'bar' ? (
                  <ComposedChart data={chartData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="date" tick={{ fontSize: isMobile ? 7 : 9 }} />
                    <YAxis 
                      yAxisId="left" 
                      tick={{ fontSize: isMobile ? 7 : 9, fill: chartPrimaryColor }} 
                      tickFormatter={(v) => {
                    ...
                    {showSecondaryMetric && (
                      <YAxis 
                        yAxisId="right" 
                        orientation="right" 
                        tick={{ fontSize: isMobile ? 7 : 9, fill: chartSecondaryColor }} 
                        tickFormatter={(v) => {
                    ...
                    <Tooltip 
                      contentStyle={{ fontSize: isMobile ? 9 : 11, backgroundColor: 'white', border: '1px solid #e5e7eb' }}
        ...
        {includeChart2 && chart2Data.length > 0 && (
          <div>
            ...
            <div id="pdf-chart2-preview" className="bg-white rounded border p-2" style={{ height: isMobile ? 140 : 180 }}>
              <ResponsiveContainer width="100%" height="100%">
                {chart2Type === 'bar' ? (
                  <ComposedChart data={chart2Data}>
                    ...
                    <YAxis 
                      yAxisId="left" 
                      tick={{ fontSize: isMobile ? 7 : 9, fill: chart2PrimaryColor }} 
                      tickFormatter={(v) => {
                    ...
                    {showChart2Secondary && (
                      <YAxis 
                        yAxisId="right" 
                        orientation="right" 
                        tick={{ fontSize: isMobile ? 7 : 9, fill: chart2SecondaryColor }} 
                        tickFormatter={(v) => {
        ...
        {includeDemographics && hasDemoData && (
          <div>
            <h2 className="text-[10px] md:text-xs font-semibold text-gray-900 mb-2">Dados Demográficos</h2>
            <div className="grid grid-cols-2 gap-1 md:gap-2">
        ...
        {includeCampaigns && topCampaigns.length > 0 && (
          <div>
            ...
            <div id="pdf-campaigns-table" className="bg-white rounded-lg border overflow-hidden shadow-sm overflow-x-auto">
              <table className="w-full text-[8px] md:text-[9px]">
                <thead>
                  <tr style={{ backgroundColor: primaryColor }}>
                    <th className="text-left px-1.5 md:px-2 py-1.5 md:py-2 font-bold text-white w-6 md:w-8">#</th>
                    <th className="text-left px-1.5 md:px-2 py-1.5 md:py-2 font-bold text-white">Campanha</th>
                    <th className="text-right px-1.5 md:px-2 py-1.5 md:py-2 font-bold text-white">Gasto</th>
                    <th className="text-right px-1.5 md:px-2 py-1.5 md:py-2 font-bold text-white hidden sm:table-cell">
                      {businessModel === 'inside_sales' ? 'Leads' : businessModel === 'pdv' ? 'Visitas' : 'Compras'}
                    </th>
                    <th className="text-right px-1.5 md:px-2 py-1.5 md:py-2 font-bold text-white">
                ...
                <tbody>
                  {topCampaigns.map((c, idx) => (
                    <tr key={c.campaign_id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/70'}>
                      <td className="px-1.5 md:px-2 py-1.5 md:py-2 text-center font-bold text-gray-700">{idx + 1}</td>
                      <td className="px-1.5 md:px-2 py-1.5 md:py-2 text-gray-800 truncate max-w-[80px] md:max-w-[120px] font-medium" title={c.campaign_name}>{c.campaign_name}</td>
                      <td className="text-right px-1.5 md:px-2 py-1.5 md:py-2 text-gray-800 font-semibold">{fmtCurrency(c.spend, currency)}</td>
                      <td className="text-right px-1.5 md:px-2 py-1.5 md:py-2 text-gray-800 hidden sm:table-cell">{fmtNumber(c.conversions)}</td>
                      <td className="text-right px-1.5 md:px-2 py-1.5 md:py-2 font-bold" style={{ color: primaryColor }}>
      ...
      <div className="px-3 md:px-4 py-1.5 md:py-2 text-center" style={{ backgroundColor: primaryColor }}>
        <p className="text-[9px] md:text-[10px] text-white font-medium">{fmtDateRange(activePeriod.since, activePeriod.until)}</p>
        <p className="text-[8px] md:text-[9px] text-white/70">{projectName} • Relatório gerado automaticamente</p>
      </div>
    </div>
  );
      ...
      <Drawer open={open} onOpenChange={setOpen}>
        ...
        <DrawerContent className="h-[90vh] flex flex-col">
          <DrawerHeader className="pb-2 shrink-0">
            <DrawerTitle>Construtor de PDF</DrawerTitle>
          </DrawerHeader>
          
          <Tabs value={mobileTab} onValueChange={(v) => setMobileTab(v as 'config' | 'preview')} className="flex-1 flex flex-col min-h-0">
            <TabsList className="grid w-full grid-cols-2 mx-4 shrink-0" style={{ width: 'calc(100% - 32px)' }}>
            ...
            <TabsContent value="config" className="flex-1 overflow-y-auto px-4 py-2 m-0">
              {ConfigPanel}
            </TabsContent>
            
            <TabsContent value="preview" className="flex-1 overflow-y-auto px-4 py-2 m-0 bg-muted/30">
              {(loading || demoLoading) && (
