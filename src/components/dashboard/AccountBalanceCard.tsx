...
export function AccountBalanceCard({ projectId, currency = 'BRL' }: AccountBalanceCardProps) {
  ...
  const getFundingTypeInfo = (type: number | null) => {
    switch (type) {
      case 1: return { label: 'Cartão de Crédito', icon: CreditCard };
  ...
  const getAccountStatusInfo = (status: number | null) => {
    ...
    // 101 = CLOSED
    switch (status) {
      case 1: return { label: 'Conta ativa', variant: 'default' as const, icon: CheckCircle2 };
      case 2: return { label: 'Conta desabilitada', variant: 'destructive' as const, icon: XCircle };
      case 3: return { label: 'Conta pausada - Sem saldo', variant: 'destructive' as const, icon: PauseCircle };
      case 7: return { label: 'Em análise', variant: 'secondary' as const, icon: Clock };
      case 9: return { label: 'Período de carência', variant: 'secondary' as const, icon: Clock };
      case 100: return { label: 'Fechamento pendente', variant: 'destructive' as const, icon: AlertTriangle };
      case 101: return { label: 'Conta fechada', variant: 'destructive' as const, icon: XCircle };
      default: return null;
    }
  };
      ...
      <div className="pt-4 pb-4 px-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {accountStatusInfo && StatusIcon && (
              <Badge variant={accountStatusInfo.variant} className="text-xs gap-1">
            ...
            {data?.daysOfSpendRemaining !== null && data?.daysOfSpendRemaining !== undefined && data.status !== 'critical' && (
              <Badge variant={
            ...
            {data?.autoReloadEnabled && (
              <Badge variant="outline" className="text-xs gap-1 text-metric-positive border-metric-positive/30">
        ...
        {data?.status === 'critical' && (
          <div className="mt-3 p-2 rounded-md bg-destructive/10 border border-destructive/20 flex items-center gap-2">
        ...
        {/* Average Daily Spend */}
        {data?.avgDailySpend > 0 && data?.status !== 'critical' && (
