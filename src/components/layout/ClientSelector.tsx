...
import { ChevronDown, Search, Check, Settings, Pencil, Archive } from 'lucide-react';
              ...
              filteredProjects.map((project) => (
                ...
                >
                  ...
                  <button
                    onClick={async (e) => {
                      e.stopPropagation();
                      if (window.confirm(`Deseja arquivar o projeto ${project.name}?`)) {
                        await archiveProject(project.id);
                      }
                    }}
                    className="p-1 rounded hover:bg-destructive/10 group/archive transition-colors flex-shrink-0"
                    title="Arquivar projeto"
                  >
                    <Archive className="w-3 h-3 text-muted-foreground group-hover/archive:text-destructive" />
                  </button>
                </div>
              ))
            )}
